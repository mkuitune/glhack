...
struct SimplexNoise
{
    struct Grad
    {
        const double x_, y_, z_, w_;

        Grad(double x, double y, double z):x_(x), y_(y), z_(z), w_(0.0){}
        Grad(double x, double y, double z, double w):x_(x), y_(y), z_(z), w_(w){}
    };

    typedef int16_t              index_t;
    typedef std::vector<index_t> index_array_t;
    typedef std::vector<Grad>    gradient_array_t;

    static double dot(const Grad& g, const double x, const double y){
        return g.x_ * x + g.y_ * y;
    }
    static double dot(const Grad& g, const double x, const double y, const double z){
        return g.x_ * x + g.y_ * y + g.z_ * z;
    }
    static double dot(const Grad& g, const double x, const double y, const double z, const double w){
        return g.x_ * x + g.y_ * y + g.z_ * z + g.w_ * w;
    }

    struct Impl_s
    {
        gradient_array_t grad3;
        gradient_array_t grad4;
        index_array_t    perm;
        index_array_t    perm_mod12;
        ...
        static const int perm_table_size = 512;
        ...
        Impl_s()
        {
            ...
            Grad sx_grad3[] = {Grad(1,1,0),Grad(-1,1,0),Grad(1,-1,0),Grad(-1,-1,0),
                      Grad(1,0,1),Grad(-1,0,1),Grad(1,0,-1),Grad(-1,0,-1),
                      Grad(0,1,1),Grad(0,-1,1),Grad(0,1,-1),Grad(0,-1,-1)};

            Grad sx_grad4[]= {Grad(0,1,1,1),Grad(0,1,1,-1),Grad(0,1,-1,1),Grad(0,1,-1,-1),
            ...
            for(auto& g3 : sx_grad3) grad3.push_back(g3);
            for(auto& g4 : sx_grad4) grad4.push_back(g4);

            perm.resize(perm_table_size);
            perm_mod12.resize(perm_table_size);

            index_t p[] = {151,160,137,91,90,15,
            ...
            for(int i=0; i<512; i++) {
                perm[i]       = p[i & 255];
                perm_mod12[i] = (index_t)(perm[i] % 12);
            }
        }
...
{
    return SimplexNoise::impl.noise(x, y);
}

double simplex_noise(double x, double y, double z)
{
    return SimplexNoise::impl.noise(x, y, z);
}

double simplex_noise(double x, double y, double z, double w)
{
    return SimplexNoise::impl.noise(x, y, z, w);
}
