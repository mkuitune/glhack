...
*/
void convert_from_sRGB(float* buf, const unsigned char* from, int W, int delta)
{
  initialize();
  from += (W-1)*delta;
  buf += W;
...
/*! Converts linear file data to linear floating point. The input data is
    ...
    arrays may be the same memory if \a delta is 4 or less.
*/
void convert_from_linear(float* buf, const unsigned char* from, int W, int delta)
{
  from += (W-1)*delta;
  buf += W;
  for (; --W >= 0; from -= delta) *--buf = *from*(1.0f/255.f);
}
...
void convert_to_linear(unsigned char* buf, const float* from, int W, int delta)
{
  ...
  /* go fowards from starting point to end of line: */
  float error = .5;
  for (p = buf+start*delta, q = from+start; p < end; p += delta, q++) {
  ...
  /* go backwards from starting point to start of line: */
  error = .5;
  for (p = buf+(start-1)*delta, q = from+start - 1; p >= buf; p -= delta, q--) {
    float G = error + *q * 255.0f;
    if (G <= 0) {
      *p = 0;
    } else if (G < 255) {
  ...
  for (p = buf + start * delta, q = from + start; p < end; p += delta, q++) {
    ...
    if (G <= 0.f) {
      *p = 0;
      ...
      error = modff(G, &intgr); // set error as the fractional part
      *p = lrintf(intgr);
    } else {
      *p = 255;
    }
